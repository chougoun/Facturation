...
//-----------------------// Controle de saisie champ nom famille //----------------------//
...
//-------------------//Controle de saisie champs ID famille //-------------------// 
...
//-------------------//Saisie incorrect ID famille//----------------------//
  if (!empty($_POST["fam_id_famille"])) {
     if (!is_numeric($_POST["fam_id_famille"]))  {
       $famIdFamilleErr = "Le champs ID famille ne peut être composé que de chiffres";
     }  
  } 
}
...
if($famIdFamilleErr or $famNomFamilleErr != "") {
	...
	$_SESSION['famNomFamilleErr'] = $famNomFamilleErr;
	   ...
	   modifierAccept($famIdFamille,$famNomFamille);
